...
function Article() {
  ...
  return (
    <>
      <Navbar />
      <main>
        <nav className="px-40 bg-white py-5 w-full border-b border-slate-300 mt-20">
          <button onClick={() => navigate("/")} className="font-semibold text-xl cursor-pointer">
            fazztrack
          </button>
        </nav>
        <div className="grid grid-cols-[1fr_3fr_1fr] gap-10 mt-10 px-40">
          <div className="flex flex-col gap-2">
            <img className="h-10 w-10 border border-slate-300" src="/public/img/logo-fazztrack.jpeg" alt="" />
            <div className="text-[#6B6B6B]">Belajar menjadi software engineer secara online/remote selama 3–6 bulan sampai diterima kerja, tanpa bayar di depan (ISA)</div>
          ...
          <article>
            <header>
              <h1 className="font-bold text-5xl mb-10">{dataArticles.title}</h1>
              <div className="flex gap-4 items-center">
                <div className="rounded-full overflow-hidden w-10 h-10">
                  <img src={dataArticles.creator.avatar} alt={dataArticles.creator.name} />
                </div>
                <div>{dataArticles.creator.name}</div>
                <button className="px-3 py-1 rounded-full border">Follow</button>
